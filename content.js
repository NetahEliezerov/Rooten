    ...
    if(document.getElementsByClassName('previewModal--player-titleTreatment-logo').length != 0) {
        ...
        chrome.storage.sync.get("openWithRootenButtonColor", function (value) {
            ...
            chrome.storage.sync.get("platformToOpenWith", function (platformValue) {
                let platform = 0;
                if(platformValue.platformToOpenWith) {
                    if(platformValue.platformToOpenWith == 'rotten') {
                        platform = 0;
                    } else if(platformValue.platformToOpenWith == 'letterboxd') {
                        platform = 1;
                    }
                }
